...
/* Details data */
$array_fields 	= array('*');
$table 	   		= 'tbl_oqc_lon oqc_lon';
$joins 	   		= 'LEFT JOIN tbl_oqc_lon_capa_monitoring capa_monitoring ON capa_monitoring.oqc_lon_id =oqc_lon.pkid';
$sql_order 		= 'ORDER BY oqc_lon.date_inspected';
$sql_limit 		= '';
$return = array();

// $sql_where 		= 'WHERE (oqc_lon.date_inspected BETWEEN "'.$date_from.'-01" AND "'.$date_to.'-31") AND oqc_lon.logdel=0 AND oqc_lon.pkid = 69';
...
LEFT JOIN tbl_oqc_lon_capa_monitoring capa_monitoring ON capa_monitoring.oqc_lon_id =oqc_lon.pkid 
WHERE (oqc_lon.date_inspected BETWEEN "2024-01-01" AND "2024-07-31") AND oqc_lon.logdel=0 
AND oqc_lon.pkid = "69"
ORDER BY oqc_lon.date_inspected ;
...
SELECT SQL_CALC_FOUND_ROWS * FROM tbl_oqc_lon oqc_lon LEFT JOIN tbl_oqc_lon_capa_monitoring capa_monitoring ON capa_monitoring.oqc_lon_id =oqc_lon.pkid WHERE (oqc_lon.date_inspected LIKE "%2024-07%") AND oqc_lon.logdel=0 ORDER BY oqc_lon.date_inspected ;
...
while($row_group = mysqli_fetch_assoc($result_group)){	
	...
	$sql_where 		= 'WHERE (oqc_lon.date_inspected LIKE "%'.$row_group['year_inspected'].'-'.sprintf("%02d", $row_group['month_inspected']).'%") AND oqc_lon.logdel=0';
	...
	while($row_details = mysqli_fetch_assoc($result_details)){
		...
		// $tbl_oqc_lon_capa_monitoring_by_id = get_tbl_oqc_lon_capa_monitoring_by_id($return['pkid']);
	...
	// echo	$col.$row.':'.$col.count($result_details);
	
	// echo $col.$first_row.':'.$col.$last_row;
}

// var_dump(count($custom_row_count));
// var_dump($last_row);
