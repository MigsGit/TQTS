...
function get_file_path_by_pkid($fkdetails) {
	...
	$array_fields = array('monitoring_file','(SELECT file_path FROM tbl_file_path WHERE pkid=fkfile_path LIMIT 0,1) as file_path');
	$table 	   	= 'tbl_ipqc_visual_inspection details ';
	$joins 	   	= '';
	...
	if($row = mysqli_fetch_array($result)){
		...
		$file['extension'] = end(explode('.',$row['monitoring_file']));
		$file['file_name'] = $row['monitoring_file'];		
	}
	return $file;
}
?>