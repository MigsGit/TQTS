<?php
	...
	$database_config = '../../db_config/config_edocapp.php';
	...
	*/
	...
	$sWhere .= "`fk_category` = 47 AND `status` = 1 AND  logdel = '0'  ";
	// $sWhere .= "`fk_category` = 47 AND `status` = 1 AND logdel = '0'  "; //display for live
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		$row = array();
		unset($row);
		$status = ($aRow['status'] == 1)? $badge = '<span class="badge highlight-color-green">APPROVED</span>':'';
		$row[] = '<center>'.$badge.'</center>';
		$row[] = date('M d, Y', strtotime($aRow['date_created']));			
		$row[] = get_category($aRow['fk_category']);
		$row[] = $aRow['document_name'];
		$row[] = $aRow['document_filename'];
		$row[] = get_emp_name($aRow['created_by']);
...
function get_category($category){
	require_once('../../class/oop_edocapp.php');

	$array_fields = array('*');
	$table = 'tbl_category';
	$joins = '';
	$sql_where = 'WHERE `pkid` = "'.$category.'" AND `logdel` = 0';
	$sql_order = '';
	$sql_limit = '';
	$result = EDOCAPP::getInstance()->select_query($array_fields,$table,$joins,$sql_where,$sql_order,$sql_limit);
	while($row = mysqli_fetch_array($result)){
		$return_category =$row['category'];
	}
	return $return_category;
}
function get_emp_name($created_by){
	require_once('../../handler/common_function.php');
	$created_by = get_emp_name_by_username_systemone($created_by);
	return $created_by;
}
?>