<?php
	...
	*/
	...
	$array_where[]  = 'fp_verfied_by = "'.$username.'"';
	if($sWhere == ""){
		$sWhere .= "WHERE  ";
	}else{
		$sWhere .= "AND ";
	}
	$sWhere .= implode(' AND ',$array_where);
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		switch($aRow['status']){
			case "0" : $badge_class = "badge highlight-color-yellow"; $badge_text = "Pending [Findings / Approval]"; break;
			case "1" : $badge_class = "badge highlight-color-blue"; $badge_text = "Pending [Analysis and Corrective Action]"; break;
			case "2" : $badge_class = "badge highlight-color-blue"; $badge_text = "Pending [Result of Confirmation]"; break;
			case "3" : $badge_class = "badge highlight-color-green";  $badge_text = "Closed"; break;
			case "9" : $badge_class = "badge highlight-color-red"; 	  $badge_text = "Disapproved "; break;
		}
		$row[] = '<span class="'.$badge_class.'" id="'.$aRow['pkid'].'">'. $badge_text."<span>";
		$row[] = $aRow['control_no'];
		$row[] = $aRow['project'];
		$row[] = $aRow['lot_no'];
		$row[] = $aRow['model'];
