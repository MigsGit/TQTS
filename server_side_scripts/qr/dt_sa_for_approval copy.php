...
function return_sa_main_approvers($fkid) {
	...
	while($row = mysqli_fetch_array($result)){
		...
		if($status == 0){
			$status = '-';
			$is_approver = '<span class="badge highlight-color-yellow">' . get_emp_name_by_username($row['approver_username']) . '</span>';
		}else if($status == 1){
			$status = 'APPROVED';
			$is_approver = get_emp_name_by_username($row['approver_username']);
		}else if($status == 2){
			$status = 'DISAPPROVED';
			$is_approver = get_emp_name_by_username($row['approver_username']);
		}
		...
		$date_time_approved = $row['date_time_approved'] == '' ? '' : '<i> - '.(date('M d, Y h:i:s A',strtotime($row['date_time_approved']))).'</i>';
		$remarks			= $row['approver_remarks'];
		$approvers .= $is_approver.'<b> ['.$status.$date_time_approved.'] </b>'.$remarks.'<br>';
	}
	return $approvers;
}
