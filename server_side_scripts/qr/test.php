<?php
	...
	*/
	...
	if($status != '') {
		...
		$sWhere .= $status. " logdel=0";
	} else {
		if($sWhere == ""){
			$sWhere .= "WHERE logdel=0";
		}else{
			$sWhere .= "AND ";
		}
	}
	...
	{	
		$buttons = '';
		$user_access = check_user_access($un,$aRow['pkid'],$aRow['status']);
		if($user_access['access'] != 'NO ACCESS') {
			$row = array();
			unset($row);
			$part_details = '';
			if($user_access['status'] == 'FOR APPROVAL') {
				$badge = '<span class="badge highlight-color-yellow" >'.$user_access['status'].'</span>';
			} else if($user_access['status'] == 'APPROVED') {
				$badge = '<span class="badge highlight-color-green" >'.$user_access['status'].'</span>';
			} else if($user_access['status'] == 'DISAPPROVED') {
				$badge = '<span class="badge highlight-color-red" >'.$user_access['status'].'</span>';
			} else if($user_access['status'] == 'WAITING DISPOSITION') {
				$badge = '<span class="badge highlight-color-lime" >'.$user_access['status'].'</span>';
			} else if($user_access['status'] == 'WITH TREATMENT (OK TO USE)' || $user_access['status'] == 'WITH TREATMENT (USE AS IS)' || $user_access['status'] == 'WITH FINAL REPLY') {
				$badge = '<span class="badge highlight-color-blue" >'.$user_access['status'].'</span>';
			} else if($user_access['status'] == 'WITH TREATMENT') {
				$badge = '<span class="badge highlight-color-purple" >'.$user_access['status'].'</span>';
			} else if($user_access['status'] == 'CANCELLED') {
				$badge = '<span class="badge highlight-color-red" >'.$user_access['status'].'</span>';
			} else {
				$badge = $user_access['status'];
			}
			...
			if($user_access['access'] == 'CREATOR') {
				if($user_access['status'] == 'APPROVED' || $user_access['status'] == 'WAITING DISPOSITION') {
					$buttons = '<button type="button" class="btn btn-default fa fa-eye" id="btn_edit" value="'.$aRow['pkid'].'"> View</button>';
				} else {
					if($user_access['status'] != 'CANCELLED') {
	...
	function check_user_access($username,$fkng,$status) {
		require_once('../../class/oop_tqts.php');
		$array_fields = array('details.created_by', 'details.approver_username', 'details.status');
		$table 	   	= 'tbl_qfr_ng_approvers details';
		$joins 	   	= 'INNER JOIN tbl_qfr_ng main ON details.fkng = main.pkid';
		$sql_where 	= 'WHERE (details.created_by = "'.$username.'" OR details.approver_username = "'.$username.'") AND details.fkng="'.$fkng.'" AND details.logdel=0 AND main.logdel=0';
		$sql_order 	= '';
		$sql_limit 	= 'LIMIT 0,1';
        $user       = array();
		...
		if($result->num_rows == 0) {
			...
			if($row=mysqli_fetch_array($result)) {
				...
				if($creator == $username) {
					$user['access'] = 'CREATOR';
				} else if($approver_username == $username){
					$user['access'] = 'APPROVER';
				} else if($creator == $username && $approver_username == $username) {
					$user['access'] = 'BOTH';
				} 
				// $user['status'] = ($approver_status == 'PENDING' ? 'FOR APPROVAL' : $status);
				$user['status'] = ($approver_status == 'PENDING' ? 'FOR APPROVAL' : $approver_status);
			}
		}
		return $user;
	}
