<?php
	...
	*/
	...
	$sWhere .= " (verified_conformed_by_lqc LIKE '%".$_GET['username']."%' OR pmi_orginator_fill_in_checked_by LIKE '%".$_GET['username']."%') AND logdel=0";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		unset($row);
		$row   = array();
		if($aRow['verified_conformed_by_lqc_logs'] == 'PENDING') {
			$badge 	   = '<span class="badge highlight-color-yellow" > PENDING</span>';
		} else if(strstr($aRow['verified_conformed_by_lqc_logs'],'ACCEPT')) {
			$badge 	   = '<span class="badge highlight-color-green" > ACCEPT</span>';
		} else if(strstr($aRow['verified_conformed_by_lqc_logs'],'REJECT')) {
			$badge 	   = '<span class="badge highlight-color-red" > REJECT</span>';
		} else {
			$badge 	   = '<span class="badge highlight-color-red" > '.$aRow['verified_conformed_by_lqc_logs'].'</span>';
		}
		
		if(($aRow['verified_conformed_by_lqc'] == $aRow['pmi_orginator_fill_in_checked_by']) || ($aRow['verified_conformed_by_lqc'] == $aRow['pmi_orginator_fill_in_approved_by_logs']) ) {
			if(($aRow['pmi_orginator_fill_in_checked_by_logs'] == 'PENDING')) {
				$badge 	   = '<span class="badge highlight-color-yellow" > PENDING </span>';
			} else if($aRow['pmi_orginator_fill_in_checked_by_logs'] == 'APPROVED') {
				$badge 	   = '<span class="badge highlight-color-green" > APPROVED</span>';
			} else if($aRow['pmi_orginator_fill_in_checked_by_logs'] == 'DISAPPROVED') {
				$badge 	   = '<span class="badge highlight-color-red" > DISAPPROVED</span>';
			} else if(strstr($aRow['pmi_orginator_fill_in_checked_by_logs'], 'CHECKED')) {
				$badge 	   = '<span class="badge highlight-color-green" > CHECKED</span>';
			} else {
				$badge 	   = '<span class="badge highlight-color-red" > NOT FOUND!</span>';
			}
		}
		...
		$verified_conformed_by_lqc  = display_qcfr_status($aRow['verified_conformed_by_lqc'], $aRow['verified_conformed_by_lqc_logs']);
		$verified_conformed_by_prdn = display_qcfr_status($aRow['verified_conformed_by_prdn'], $aRow['verified_conformed_by_prdn_logs']);
		$verified_conformed_by_eng  = display_qcfr_status($aRow['verified_conformed_by_eng'], $aRow['verified_conformed_by_eng_logs']);
		$approved_by_sh   			= display_qcfr_status($aRow['approved_by_sh'], $aRow['approved_by_sh_logs']);
		$approved_by_dh   			= display_qcfr_status($aRow['approved_by_dh'], $aRow['approved_by_dh_logs']);
		$originator_checked_by   	= display_qcfr_status($aRow['pmi_orginator_fill_in_checked_by'], $aRow['pmi_orginator_fill_in_checked_by_logs']);
		$originator_approved_by		= display_qcfr_status($aRow['pmi_orginator_fill_in_approved_by'], $aRow['pmi_orginator_fill_in_approved_by_logs']);
