<?php
	...
	*/
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		$is_approver  	= check_is_approver($username, $aRow['pkid']);
		...
		$status_array  	= return_status($username, $aRow['pkid'], $aRow['status'], $approver_array['fk_attachment']);
		$status			= $status_array['status'];
		$action			= $status_array['action'];
		
		if($aRow['created_by'] == $username || $is_approver) {
			...
			} else if($status == 'FOR UPLOAD') {
				$badge = '<span class="badge highlight-color-blue" >'.$status.'</span>';
			} else if($status == 'FOR SEND') {
				$badge = '<span class="badge highlight-color-lime" >'.$status.'</span>';
			} else {
				$badge = '<span class="badge highlight-color-default" >'.$status.'</span>';
			}
			
			if($aRow['created_by'] == $username && $aRow['status'] == 'OPEN') {
				$buttons  = '<button type="button" style="margin-bottom:5px;" class="btn btn-primary fa fa-edit" data-id="'.$aRow['pkid'].'" value="'.$aRow['pkid'].'"> Edit</button>';
			} else if($aRow['created_by'] == $username && $aRow['status'] == 'FOR UPLOAD') {
				// /* $buttons .= '<button type="button" style="margin-bottom:5px;" class="btn btn-primary fa fa-pencil-square-o" data-id="'.$aRow['pkid'].'" id="" value="'.$aRow['pkid'].'"> Revise</button><br> '; */
				$buttons .= '<button type="button" style="margin-bottom:5px;" class="btn btn-primary fa fa-edit" data-id="'.$aRow['pkid'].'" id="" value="'.$aRow['pkid'].'"> Revise</button><br> ';
				$buttons .= '<button type="button" style="margin-bottom:5px;" class="btn btn-success fa fa-upload" data-id="'.$aRow['pkid'].'" id="" value="'.$aRow['pkid'].'"> Upload</button>';
			} else if($aRow['created_by'] == $username && $aRow['status'] == 'FOR SEND') {
			...
			if($is_approver && $aRow['status'] != 'FOR SEND') {
				if($is_approver && ($status == 'OPEN' || $status == 'PENDING')) {
					$buttons .= '<button type="button" style="margin-bottom:5px;" class="btn btn-primary fa fa-commenting-o" data-id="'.$aRow['pkid'].'" id="" value="'.$aRow['pkid'].'"> Add Comment</button>';
				} else if($is_approver && $aRow['status'] == 'FOR UPLOAD') {
					$buttons .= '<button type="button" style="margin-bottom:5px;" class="btn btn-primary fa fa-commenting-o" data-id="'.$aRow['pkid'].'" id="" value="'.$aRow['pkid'].'"> Edit Comment</button>';
					$buttons .= '<button type="button" style="margin-bottom:5px;" class="btn btn-default fa fa-eye" data-id="'.$aRow['pkid'].'" id="" value="'.$aRow['pkid'].'"> View</button>';
				} else if($status == 'DONE' && $action == 'add_update') {
					$buttons .= '<button type="button" style="margin-bottom:5px;" class="btn btn-primary fa fa-commenting-o" data-id="'.$aRow['pkid'].'" id="" value="'.$aRow['pkid'].'"> Edit Comment</button>';
				} else if($status == 'CLOSED' && $action == 'add_update') {
					$buttons .= '<button type="button" style="margin-bottom:5px;" class="btn btn-primary fa fa-commenting-o" data-id="'.$aRow['pkid'].'" id="" value="'.$aRow['pkid'].'"> Edit Comment</button>';
				} else {
					$buttons .= '<button type="button" style="margin-bottom:5px;" class="btn btn-default fa fa-eye" data-id="'.$aRow['pkid'].'" id="" value="'.$aRow['pkid'].'"> View</button>';
				}
	...
	function check_is_approver($username, $fk8d) {
		require_once('../../class/oop_tqts.php');
		$array_fields = array('pkid');
		$table 	   	= 'tbl_qfr_8d_approvers';
		$joins 	   	= '';
		$sql_where 	= 'WHERE approver_username="'.$username.'" AND fk8d="'.$fk8d.'" AND logdel=0';
		$sql_order 	= '';
		$sql_limit 	= 'LIMIT 0,1';
		$result = TQTS::getInstance()->select_query($array_fields,$table,$joins,$sql_where,$sql_order,$sql_limit);
		$script = TQTS::getInstance()->select_query_script($array_fields,$table,$joins,$sql_where,$sql_order,$sql_limit);
	...
	function return_status($username, $fk8d, $main_status, $fk8d_attachment_initial) {
		require_once('../../class/oop_tqts.php');
		$array_status = array();
		$array_fields = array('status');
		$table 	   	= 'tbl_qfr_8d_approvers';
		$joins 	   	= '';
		$sql_where 	= 'WHERE approver_username="'.$username.'" AND fk8d="'.$fk8d.'" AND fk8d_attachment_initial="'.$fk8d_attachment_initial.'" AND logdel=0';
		$sql_order 	= '';
		$sql_limit 	= 'LIMIT 0,1';
		$result = TQTS::getInstance()->select_query($array_fields,$table,$joins,$sql_where,$sql_order,$sql_limit);
		$script = TQTS::getInstance()->select_query_script($array_fields,$table,$joins,$sql_where,$sql_order,$sql_limit);
		if($row = mysqli_fetch_array($result)) {
			$array_status['status'] = $row['status'];
			$array_status['action'] = 'add_update';
		} else {
			$array_status['status'] = $main_status;
			$array_status['action'] = 'view';
			$sql_where 	= 'WHERE approver_username="'.$username.'" AND fk8d="'.$fk8d.'" AND logdel=0';
			$result = TQTS::getInstance()->select_query($array_fields,$table,$joins,$sql_where,$sql_order,$sql_limit);
			if($row = mysqli_fetch_array($result)) {
				$array_status['status'] = $row['status'];
