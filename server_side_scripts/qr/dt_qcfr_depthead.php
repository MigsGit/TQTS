<?php
	...
	*/
	...
	$sWhere .= " approved_by_dh LIKE '%".$_GET['username']."%' AND approved_by_dh_logs != '' AND logdel=0";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		unset($row);
		$row   = array();
		if($aRow['approved_by_dh_logs'] == 'PENDING') {
			$badge 	   = '<span class="badge highlight-color-yellow" > PENDING</span>';
		} else if(strstr($aRow['approved_by_dh_logs'],'APPROVED')) {
			$badge 	   = '<span class="badge highlight-color-green" > APPROVED</span>';
		} else if(strstr($aRow['approved_by_dh_logs'],'DISAPPROVED')) {
			$badge 	   = '<span class="badge highlight-color-red" > DISAPPROVED</span>';
		} else {
			$badge 	   = '<span class="badge highlight-color-red" > '.$aRow['approved_by_dh_logs'].'</span>';
		}
		
		if($aRow['approved_by_dh_logs'] == $aRow['pmi_orginator_fill_in_approved_by'] ) {
			if($aRow['pmi_orginator_fill_in_approved_by_logs'] == 'PENDING') {
				$badge 	   = '<span class="badge highlight-color-yellow" > PENDING</span>';
			} else if($aRow['pmi_orginator_fill_in_approved_by_logs'] == 'APPROVED') {
				$badge 	   = '<span class="badge highlight-color-green" > APPROVED</span>';
			} else if($aRow['pmi_orginator_fill_in_approved_by_logs'] == 'DISAPPROVED') {
				$badge 	   = '<span class="badge highlight-color-red" > DISAPPROVED</span>';
			} else {
