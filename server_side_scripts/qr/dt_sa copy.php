<?php
	...
	*/
	...
	$sWhere .= " (`status` = '0' OR `status` = '2' OR `status` = '8' OR `status` = 'A') AND `logdel` = 0";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		 ...
		 */
		if($aRow['status'] == 'A') {
			$badge = '<span class="badge highlight-color-blue">FOR QC CHECKING</span>';
		}
		if($aRow['status'] == '0') {
			...
			$badge = '<span class="badge highlight-color-lime">FOR APPROVAL</span>';
		...
		$row[] = $originators;
		$row[] = $aRow['supplier'];
		$row[] = '<center><a class= "btn-link fa fa-paperclip" href="#" id="a_download_excel" data-id="'.$aRow['pkid'].'"> Download Excel</a></center>';
		$qc_approvers = return_sa_approvers_qc($aRow['pkid']);
		$qc_approvers_status = return_sa_is_decision_qc($aRow['pkid']);
		$approvers = return_sa_approvers($aRow['pkid']);
		$pmi_approvers = return_sa_main_approvers($aRow['pkid']);

		if($qc_approvers_status == '0'){
			$qc_approvers='<span class="badge highlight-color-blue">'.$qc_approvers.'</span>';
			$approvers=$approvers;
		}else if($qc_approvers_status == '1'){
			$qc_approvers=$qc_approvers;
			$approvers='<span class="badge highlight-color-yellow">'.$approvers.'</span>';
		}else{
			$approvers='<span class="badge highlight-color-yellow">'.$approvers.'</span>';
		}

		$row[] = $qc_approvers.'<br>'.$approvers;
		$row[] = $pmi_approvers;
		...
		if($aRow['status'] == '0'){
			...
			if( $user_sa_access['update'] && $username == $aRow['created_by']){
		...
		else if($aRow['status']=='A'){
			/* check user access */
			$button[] = '<center>';
			if($username == $aRow['judged_by_qc']){
				$button[] = '<button class="btn btn-info fa fa-search" style="margin-bottom:5px;" id="'.$aRow['pkid'].'">Check</button>';
			}
			// if($user_sa_access['update'] && $username == $aRow['created_by']){
				$button[] = '<button class="btn btn-default fa fa-eye" style="margin-bottom:5px;" id="'.$aRow['pkid'].'"> View</button>';
				$button[] = '<button class="btn btn-primary fa fa-edit" style="margin-bottom:5px;" id="'.$aRow['pkid'].'"> Edit</button>';
			...
			/* if request has already been closed ============= CONTINUE THIS FUNCTION COPY THE PROCESS OF NG Report */ 
			$button[] = '<center>';
			if($aRow['status']=='2' && $username == $aRow['created_by']){
				$button[] = '<button class="btn btn-warning fa fa-refresh" style="margin-bottom:5px;" id="'.$aRow['pkid'].'"> Revise </button>';
			}
...
function return_sa_approvers($fkid) {
	...
	while($row = mysqli_fetch_array($result)){
		...
		$approvers .= get_emp_name_by_username($row['approver_username']).'<b> ['.$status.$date_time_approved.'] </b>'.$remarks.'<br>';
	}
	return $approvers;
}
...
function return_sa_main_approvers($fkid) {
	require_once('../../class/oop_tqts.php');
	require_once('../../handler/common_function.php');
	// return 'true';
	$array_fields = array('approver_username', 'status','date_time_approved','approver_remarks','created_by');
