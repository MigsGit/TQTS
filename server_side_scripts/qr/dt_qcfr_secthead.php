<?php
	...
	*/
	...
	$sWhere .= " (approved_by_sh LIKE '%".$_GET['username']."%' OR pmi_orginator_fill_in_approved_by LIKE '%".$_GET['username']."%') AND approved_by_sh_logs != '' AND logdel=0";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		unset($row);
		$row   = array();
		if($aRow['approved_by_sh_logs'] == 'PENDING') {
			$badge 	   = '<span class="badge highlight-color-yellow" > PENDING</span>';
		} else if(strstr($aRow['approved_by_sh_logs'],'DISAPPROVED')) {
			$badge 	   = '<span class="badge highlight-color-red" > DISAPPROVED</span>';
		} else if(strstr($aRow['approved_by_sh_logs'],'APPROVED')) {
			$badge 	   = '<span class="badge highlight-color-green" > APPROVED</span>';
		} else {
			$badge 	   = '<span class="badge highlight-color-red" > '.$aRow['approved_by_sh_logs'].'</span>';
		}
		
		if($aRow['pmi_orginator_fill_in_approved_by_logs'] == 'PENDING') {
			$badge 	   = '<span class="badge highlight-color-yellow" > PENDING </span>';
		}
		
		/* if( ($username == $aRow['pmi_orginator_fill_in_approved_by'] )) {
			if($aRow['pmi_orginator_fill_in_approved_by_logs'] == 'PENDING') {
				$badge 	   = '<span class="badge highlight-color-yellow" > PENDING </span>';
			} else if(strstr($aRow['pmi_orginator_fill_in_approved_by_logs'],'CHECKING')) {
				$badge 	   = '<span class="badge highlight-color-yellow" > '.$aRow['status'].'</span>';
			} else if(strstr($aRow['pmi_orginator_fill_in_approved_by_logs'],'APPROVED')) {
				$badge 	   = '<span class="badge highlight-color-green" > APPROVED</span>';
			} else if(strstr($aRow['pmi_orginator_fill_in_approved_by_logs'],'DISAPPROVED')) {
				$badge 	   = '<span class="badge highlight-color-red" > DISAPPROVED</span>';
			} else {
