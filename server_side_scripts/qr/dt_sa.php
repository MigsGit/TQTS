<?php
	...
	*/
	...
	// $sWhere .= " (`status` = 1 OR `status` = 2 OR `status` = 8) AND `logdel` = 0";
	$sWhere .= " (`status` != 6 AND `status` != 7 ) AND `logdel` = 0";
	// $sWhere .= ' `created_by`="'.$username.'" ';
	/*
	 * SQL queries
	 * Get data to display http://rapid/TQTS_TS/reports/excel_qfr_sa_for_disposition_download.php?id=10
	 */
	$sQuery = "
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		$button = array();
			...
			if($aRow['status'] == '1' ){
				...
				if($user_sa_access['delete']  && $username == $aRow['created_by']){
			...
			else if($aRow['status']=='1' || $aRow['status']=='5'){
				/* check user access */
				$button[] = '<center>';
