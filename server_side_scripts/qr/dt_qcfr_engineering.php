<?php
	...
	*/
	...
	$sWhere .= " verified_conformed_by_eng LIKE '%".$_GET['username']."%' AND logdel=0";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		unset($row);
		$row   = array();
		if(strstr($aRow['verified_conformed_by_eng_logs'],'PENDING')) {
			$badge 	   = '<span class="badge highlight-color-yellow" > PENDING</span>';
		} else if(strstr($aRow['verified_conformed_by_eng_logs'],'ACCEPT')) {
			$badge 	   = '<span class="badge highlight-color-green" > ACCEPT</span>';
		} else if(strstr($aRow['verified_conformed_by_eng_logs'],'REJECT')) {
			$badge 	   = '<span class="badge highlight-color-red" > REJECT</span>';
		} else {
			$badge 	   = '<span class="badge highlight-color-red" > '.$aRow['verified_conformed_by_eng_logs'].'</span>';
		}
