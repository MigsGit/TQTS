...
//	 ini_set('display_errors', 1);
	...
	*/
	...
	require_once('../../handler/handler_iqc.php');
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		$button .= '<button type="button" class="btn btn-primary fa fa-edit" data-id="'.$aRow['pkid'].'"> Edit</button>';
		$button .= '<button type="button" class="btn btn-default fa fa-eye" data-id="'.$aRow['pkid'].'"> View</button>';
		
        $row[] = $aRow['month_year'] == '' ? '' : date('M, Y', strtotime($aRow['month_year']));
        $row[] = $aRow['remarks'];
        $row[] = '<button type="button" class="btn btn-link fa fa-paperclip" id="btn_attachment" value="'.$aRow['pkid'].'"> Attachments</button>';
        $row[] = $button;
