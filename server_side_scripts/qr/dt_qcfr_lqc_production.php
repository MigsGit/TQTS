<?php
	...
	*/
	...
	$sWhere .= " verified_conformed_by_prdn LIKE '%".$_GET['username']."%' AND logdel=0";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		unset($row);
		$row   = array();
		$prdn_array 	 = explode(',', $aRow['verified_conformed_by_prdn']);
		$prdn_logs_array = explode(',', $aRow['verified_conformed_by_prdn_logs']);
		$index			 = array_search($username, $prdn_array);
		$status			 = $prdn_logs_array[$index];
		
		if(strstr($status, 'PENDING') || $status == '' ) {
			$badge 	   = '<span class="badge highlight-color-yellow" > PENDING</span>';
		} else if(strstr($status, 'ACCEPT')) {
			...
			$badge 	   = '<span class="badge highlight-color-red" > '.$status.'</span>';
		}
