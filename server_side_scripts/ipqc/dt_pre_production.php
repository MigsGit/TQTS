<?php
	...
	*/
	...
	$user_pp_access 	    	= array("create" => false, 
									"read" => false,
									"update" => false,
									"delete" => false
									);
	foreach($user_role['subsystem_code'] as $key => $subsystem_code ){
		if($subsystem_code == $subsystem_code && $user_role['module'][$key] == $module && $user_role['role'][$key]=='SUPERVISOR'){ 
			if ($user_role['create'][$key] == 1){
				$user_pp_access['create'] = true;
			}
			if ($user_role['read'][$key] == 1){
				$user_pp_access['read'] = true;
			}
			if ($user_role['update'][$key] == 1){
				$user_pp_access['update'] = true;
			}
			if ($user_role['delete'][$key] == 1){
				$user_pp_access['delete'] = true;
			}
		}
	}
	/*
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		$row[] = return_sa_originators($aRow['pkid'],$aRow['checked_by']);
		$row[] = return_sa_originators($aRow['pkid'],$aRow['approved_by']);
	...
	function return_sa_originators($fkid,$username) {
		...
		while($row = mysqli_fetch_array($result)){
			$approvers .= get_emp_name_by_username_systemone_rapid($username);
