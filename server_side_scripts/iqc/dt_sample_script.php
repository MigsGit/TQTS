...
//	 ini_set('display_errors', 1);
	...
	$aColumns = array( 
				...
				'ng_condition_files',
				'pkid',
				'CONCAT("QAR-TS-",`section`,"-",DATE_FORMAT(`date_created`,"%y%m"),"-",`control_no_count`) as control_no'
				);
	
	/* used this field for searching data typed in the search box */
	// $array_search = array('part_code','part_name','lot_name','model','ng_condition_text');
	...
	*/
	...
	echo 'Search :'.json_encode($_GET['sSearch'])."<br>";
	echo 'Display Start :'.json_encode($_GET['iDisplayStart'])."<br>";
	echo 'Display Length :'.json_encode($_GET['iDisplayLength'])."<br>";
	echo 'Sorting :'.json_encode($_GET['iSortCol_0'])."<br>";
	...
	$user_iqc_qar_access 	    = array("create" => false, 
										"read" => false,
										"update" => false,
										"delete" => false
										);
	...
	foreach($user_role['subsystem_code'] as $key => $subsystem_code ){
		if($subsystem_code == $subsystem_code && $user_role['module'][$key] == $module){ 
			if ($user_role['create'][$key] == 1){
				$user_iqc_qar_access['create'] = true;
			}
			if ($user_role['read'][$key] == 1){
				$user_iqc_qar_access['read'] = true;
			}
			if ($user_role['update'][$key] == 1){
				$user_iqc_qar_access['update'] = true;
			}
			if ($user_role['delete'][$key] == 1){
				$user_iqc_qar_access['delete'] = true;
			}
		}
	}
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		switch($aRow['status']){
			...
			case "1"	: $status = '<span class="badge highlight-color-green"> Conformed & Sent</span>'; break;
		...
		if($user_iqc_qar_access['update'] && ($aRow['status'] == 0 || $aRow['status'] == 1)){
			$array_button[] = '<button style="margin-bottom:5px;" type="button" class="btn btn-primary fa fa-edit" id="btn_edit" value="'.$aRow['pkid'].'"> Edit</button>';
		}
		if($user_iqc_qar_access['delete'] && ($aRow['status'] == 0 || $aRow['status'] == 1)){
			$array_button[] = '<button style="margin-bottom:5px;" type="button" class="btn btn-danger fa fa-remove" id="btn_delete" value="'.$aRow['pkid'].'"> Cancel</button>';
		}
