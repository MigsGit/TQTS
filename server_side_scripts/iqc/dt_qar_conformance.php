...
//	 ini_set('display_errors', 1);
	...
	*/
	...
	$array_search[] = "`qc_supervisor` LIKE '%".$_GET['username']."%'";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		$status = '';
		switch($aRow['status']){
			case "0"	: $status = '<span class="badge highlight-color-yellow"> Pending Approval</span>'; break;
        ...
        $row[] = $aRow['ng_rate']."%";
        $array_button = array();
		$array_button[] = '<button style="margin-bottom:5px;" type="button" class="btn btn-default fa fa-eye" id="btn_view" value="'.$aRow['pkid'].'"> View</button>';
