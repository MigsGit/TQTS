<?php
	...
	$aColumns = array( 
				'pkid', 
				'status', 
				'IF(status = "FOR PRDN CHECKING", 0, 1) AS status_id', 
				'training_title', 
				'training_objective', 
				'line', 
				'control_no', 
	...
	*/
	...
	$sWhere .= " (status != 'DRAFT') AND prdn_checked_by='".$username."' AND logdel=0";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		if($aRow['status'] == 'FOR PRDN CHECKING') {
			...
			if($aRow['prdn_checked_by_logs'] == '') {
				$badge = '<span class="badge highlight-color-green" > APPROVED</span>'; //for previous data that was not updated
			} else {
				if($prdn_checked_by_logs[0] == 'PENDING') {
					$badge = '<span class="badge highlight-color-yellow" > FOR APPROVAL</span>';
					$buttons   = '<button type="button" class="btn btn-primary fa fa-edit" id="'.$aRow['pkid'].'" style="margin-top:5px;"> Edit</button><br>';
				} else if($prdn_checked_by_logs[0] == 'APPROVED') {
					$badge = '<span class="badge highlight-color-green" > APPROVED</span>';
				} else if($prdn_checked_by_logs[0] == 'DISAPPROVED') {
        ...
        // $row[] = $aRow['reason_for_certification']; 
        $row[] = $training_date; 
        $row[] = $signatories_log;
        $row[] = $buttons;
