...
//	 ini_set('display_errors', 1);
	...
	/* DB table to use */
	$sTable = $_GET['db']."vw_employeeinfo";
	
	$database_config = '../../db_config/config_hris.php';
	...
	*/
	...
	// }
	if($_GET['search_for'] != '') {
		$sWhere .= "WHERE ". $_GET['search_by'] . " LIKE '%".$_GET['search_for']."%' ";
	} 
	/*
	 * SQL queries
	 * Get data to display
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		switch($aRow['EmpStatus']) {
			case "Active" 		: $empno = $aRow['EmpNo']; break; 
			case "Resigned" 	: $empno = '**'.$aRow['EmpNo']; break; 
			case "Rehired" 		: $aRow['EmpNo']; break; 
			default: $empno = $aRow['EmpNo']; break;
		}
		
		if($_GET['db'] == 'db_subcon.') {
			$aRow['pkid'] = 'SUB'.$aRow['pkid'];
		}
		
        $row[] = '<center>'.$empno.'</center>';
        $row[] = $aRow['LastName'];
        $row[] = $aRow['FirstName'];
        $row[] = $aRow['MiddleName'];
        $row[] = $aRow['Position']; 
        $row[] = $aRow['Department']; 
        $row[] = '<button type="button" class="btn btn-default fa fa-eye" data-id="'.$aRow['pkid'].'"> View</button>'; 
