...
//	 ini_set('display_errors', 1);
	...
	$aColumns = array( 
				'pkid', 
				'status', 
				'IF(status = "FOR ENGR. QUALIFICATION", 0, 1) AS status_id', 
				'training_title', 
				'training_objective', 
				'line', 
				'control_no', 
	...
	*/
	...
	$sWhere .= " (prdn_checked_by != '' AND prdn_checked_by_logs LIKE 'APPROVED%') AND logdel=0";
	
	if($sOrder == ""){
		$sOrder = 'ORDER BY status_id, pkid';
	}
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		if($aRow['status'] == 'FOR ENGR. QUALIFICATION') {
		...
		} else if($aRow['status'] == 'DRAFT ENGR') {
