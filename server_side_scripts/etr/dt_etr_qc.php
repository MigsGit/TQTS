...
//	 ini_set('display_errors', 1);
	...
	$aColumns = array( 
				'pkid', 
				'status', 
				'IF(status = "FOR QC QUALIFICATION", 0, 1) AS status_id',
				'training_title', 
				'training_objective', 
				'line', 
				'control_no', 
	...
	*/
	...
	$sWhere .= " (qc_first_take_certified_by LIKE '%".$username."%' OR qc_second_take_certified_by LIKE '%".$username."%') AND (status LIKE '%QC%' OR status LIKE '%POST%') AND logdel=0";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		$signatories_log 	    = 'PRDN:'.display_approvers_log($prdn_checked_by_logs[0], $aRow['prdn_first_take_date_time'], $aRow['prdn_checked_by'], (isset($prdn_checked_by_logs[2]) ? $prdn_checked_by_logs[2] : ''));
		$signatories_log	   .= '<br>ENGR:'.display_approvers_log($engr_checked_by_logs[0], $aRow['eng_first_take_date_time'], $aRow['engr_checked_by'], (isset($engr_checked_by_logs[2]) ? $engr_checked_by_logs[2] : $engr_checked_by_logs[2]));
		$signatories_log	   .= '<br>QC:'.display_approvers_log($qc_checked_by_logs[0], $aRow['qc_first_take_date_time'], $aRow['qc_checked_by'], (isset($qc_checked_by_logs[2]) ? $qc_checked_by_logs[2] : ''));
