...
//	 ini_set('display_errors', 1);
	...
	*/
	...
	$sWhere .= " (status = 'FOR POSTING' OR status='POSTED') AND logdel=0";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		if($aRow['status'] == 'FOR POSTING') {
			$badge 	   = '<span class="badge highlight-color-yellow" > FOR POSTING</span>';
			$buttons   = '<button type="button" class="btn btn-primary fa fa-sticky-note-o" id="'.$aRow['pkid'].'" style="margin-top:5px;"> Post</button>';
		...
		$instructors = get_emp_name_by_username_systemone($aRow['prdn_first_take_trained_by']).' / ';
		$row[] = $badge;
        $row[] = $aRow['control_no'];
        $row[] = $aRow['training_title'];
        $row[] = $aRow['line'];
        $row[] = $aRow['series_name'];
        $row[] = ($aRow['prdn_first_take_date_time'] == '' ? '' : date('M d, Y', strtotime($aRow['prdn_first_take_date_time']))).' to '.($aRow['qc_first_take_date_time'] == '' ? '' : date('M d, Y', strtotime($aRow['qc_first_take_date_time']))); 
        $row[] = $instructors;
        $row[] = $aRow['venue'];
        $row[] = $buttons;
