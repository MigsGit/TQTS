...
//	 ini_set('display_errors', 1);
	...
	$sIndexColumn = "tbl_Training.pkid";
	
	/* DB table to use */
	$sTable = "tbl_Training INNER JOIN tbl_Trainee ON tbl_Trainee.fkTraining = tbl_Training.pkid";
	...
	*/
	...
	$fk_employee = $_GET['fk_employee'] == '' ? 0 : $_GET['fk_employee'];
	...
	$sWhere .= "fkEmployee='".$fk_employee."'";
	
	// if($sOrder == '') {
		// $sOrder  = 'ORDER BY fiscal_year DESC';
	// } else {
		// $sOrder .= ',fiscal_year DESC';
	// }
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		$period = ($aRow['PeriodFrom'] != ''? (date('m/d/Y', strtotime($aRow['PeriodFrom'])).'-'.date('m/d/Y', strtotime($aRow['PeriodTo']))) : 'N/A');
