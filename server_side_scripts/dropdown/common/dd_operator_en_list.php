...
/* Query Here  */
$sql  = "SELECT empno FROM vw_pmi_subcon_operators WHERE `empno` LIKE '%$search%' LIMIT 0,10";
...
if(count($list) > 0){
   foreach ($list as $key => $value) {
		$data[] = array('id' => $value['empno'], 'text' => html_entity_decode(htmlentities($value['empno'])));              
   } 
} else {
