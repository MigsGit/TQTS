...
$db_config = '../../../db_config/config_hris_views.php';
...
/* Query Here  */
$sql  = "SELECT empname FROM vw_pmi_subcon_operators WHERE `empname` LIKE '%$search%' LIMIT 0,10";
...
if(count($list) > 0){
   foreach ($list as $key => $value) {
		$data[] = array('id' => $value['empname'], 'text' => html_entity_decode(htmlentities($value['empname'])));              
   } 
} else {
