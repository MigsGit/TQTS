...
$sql  = "SELECT CONCAT(`firstname`,' ',`lastname`) as `Emp_name`,`email_add` FROM `vw_EmpInfo_Rapid` WHERE (email_add != '' AND email_add != 'kiosk@pricon.ph') AND `email_add` LIKE '%$search%' LIMIT 0,10";
...
if(count($list) > 0){
   foreach ($list as $key => $value) {
		$data[] = array('id' => $value['email_add'], 'text' => $value['Emp_name']);              
   } 
} else {
