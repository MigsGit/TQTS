...
/* Query Here  */
// $sql  = "SELECT DISTINCT `Emp_name`,`username` FROM db_acl.vw_labor_identity WHERE `Emp_name` LIKE '%$search%' LIMIT 40";
$sql  = "SELECT CONCAT(`firstname`,' ',`lastname`) as `Emp_name`,`username` FROM vw_EmpInfo_Rapid WHERE `lastname` LIKE '%$search%'  OR `firstname` LIKE '%$search%' LIMIT 0,40";
