<?php
	...
	*/
	...
	/* DB table to use */
	$sTable = "oqc_inspections";
	
	$database_config = '../../db_config/seiko_iqc_db.php';
	...
	*/
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		$row = array();
		unset($row);
		$row[] = $aRow['assembly_line'];
		$row[] = $aRow['po_no'];
		$row[] = $aRow['device_name'];
		$row[] = $aRow['lot_no'];
		$row[] = date("M d, Y",strtotime($aRow['app_date'])).'<br>'.$aRow['app_time'];
		$row[] = $aRow['date_inspected'];
		$row[] = $aRow['fy'];
		$row[] = $aRow['ww'];
		$row[] = $aRow['submission'];
		$row[] = $aRow['inspector'];
...
//         $row[] = '<button type="button" class="btn btn-link fa fa-paperclip" id="btn_attachment" value="'.$aRow['pkid'].'"> Attachments</button>';
        // $row[] = '<button type="button" class="btn btn-default fa fa-edit" id="btn_edit" value="'.$aRow['pkid'].'"> Edit</button>';
