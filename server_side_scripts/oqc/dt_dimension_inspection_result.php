...
//	 ini_set('display_errors', 1);
	...
	*/
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		$row = array();
		unset($row);
		$row[] = date('M d, Y', strtotime($aRow['shipment_date']));
        $row[] = $aRow['po_number'];
        $row[] = $aRow['series_name'];
        $row[] = $aRow['customer'];
        $row[] = $aRow['remarks'];
        $row[] = '<a href="pages/oqc/dl_oqc_dir.php?id='.$aRow['pkid'].'" class="fa fa-file-excel-o"> Download File</a>';
        $button = array();
		if($user_oqc_dir_access['read']){
			$button[] = '<button type="button" class="btn btn-default fa fa-eye" id="'.$aRow['pkid'].'" style="margin-bottom:5px;"> View</button>';
		}
		if($user_oqc_dir_access['update']){
			$button[] = '<button type="button" class="btn btn-primary fa fa-edit" id="'.$aRow['pkid'].'" style="margin-bottom:5px;"> Edit</button>';
		}
		if($user_oqc_dir_access['delete']){
			$button[] = '<button type="button" class="btn btn-danger fa fa-remove" id="'.$aRow['pkid'].'" style="margin-bottom:5px;"> Cancel</button>';
		}
		$row[] = implode('<br />',$button);
