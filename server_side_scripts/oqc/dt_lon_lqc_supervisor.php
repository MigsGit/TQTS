...
//	 ini_set('display_errors', 1);
	...
	*/
	...
	$sWhere .= " logdel=0 AND checked_by='".$username."' ";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		unset($row);
        $buttons	= '';
		if(strstr($aRow['status'] , "CANCELLED")) {
			...
			if($aRow['checked_by_status'] == 'PENDING') {
				$badge = '<span class="badge highlight-color-yellow" > PENDING</span>';
				$buttons  .= '<button type="button" class="btn btn-primary fa fa-edit" id="'.$aRow['pkid'].'"> Revise</button><br>';
			} else if($aRow['checked_by_status'] == 'ACCEPT') {
				$badge = '<span class="badge highlight-color-green" > ACCEPTED</span>';
			} else {
				$badge = '<span class="badge highlight-color-default" >'.$aRow['status'].'</span>';
			}
			if(strstr($aRow['status'], 'REJECTED') || strstr($aRow['status'], 'DISAPPROVED')) {
				$buttons   = '<button type="button" class="btn btn-primary fa fa-edit" id="'.$aRow['pkid'].'"> Revise</button><br>';
			}
			$buttons  .= '<button type="button" class="btn btn-default fa fa-eye" id="'.$aRow['pkid'].'" style="margin-top:5px;"> View</button><br>';
			$buttons  .= '<button type="button" class="btn btn-danger fa fa-close" id="'.$aRow['pkid'].'" style="margin-top:5px;"> Cancel</button><br>';
			$buttons  .= '<button type="button" class="btn btn-success fa fa-file-excel-o" id="'.$aRow['pkid'].'" style="margin-top:5px;"> Export</button>';
		}
