...
//	 ini_set('display_errors', 1);
	...
	*/
	...
	$sWhere .= " (status = 'FOR CHECKING' or status = 'CHECKED') AND tbl_theoretical_exam.logdel=0 AND tbl_theoretical_points.logdel=0";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		if($aRow['status'] == 'FOR CHECKING') {
			$badge 	  = '<span class="badge highlight-color-yellow" >'.$aRow['status'].'</span>';
			$buttons .= '<button type="button" class="btn btn-success fa fa-check" data-id="'.$aRow['pkid'].'"> Check</button><br>';
		...
		$row[] = $score . '/' . $points.' = '.(($score/$points)*100).'%';
