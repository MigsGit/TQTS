...
/***************************************************/
	...
	$('#frm_upload_ng button[name="add_lot_number"]').click(function() {
	...
	function fn_hide_show_ng_disposition(modal_id, fkng) {		
		...
		// } else if(key == 'disposition' && value != '') {
			...
			$('#' + modal_id + ' #container_ng_final_reply input[type="date"]').prop('required', true);
	...
	function fn_add_treatment(modal_id, fkng, final_reply_status, serialized_data) {
	...
	function fn_view_ng_attachments(fkng) {
		...
		call_ajax( data, handler_qfr_ng, function(result) {
			...
			$('#modal_attachment_viewer').data('id',fkng);
			$('#modal_attachment_viewer').modal('show');
		});
	}
