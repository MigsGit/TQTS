...
var tbl_special_acceptance_approval = 'tbl_special_acceptance_approval';$(document).ready(function(){

	function fn_validate_qc_supervisor(){ //modifynow active the Checked by QC select
		var data ={
			'action' 	: 'validate_qc_supervisor',
			'username'	: username
		};
		call_ajax(data,handler_qfr,function(result){
			if(result['department'] == "ENGG"){
				$('#frm_sa #txt_judged_by_qc').prop('required',true);
				$('#frm_sa .container_checked_by_qc').show();
				$('#frm_sa_edit #txt_judged_by_qc').prop('required',true);
				$('#frm_sa_edit .container_checked_by_qc').show();
			}else{
				$('#frm_sa .container_checked_by_qc').hide();
				$('#frm_sa #txt_judged_by_qc').prop('required',false);
				$('#frm_sa_edit .container_checked_by_qc').hide();
				$('#frm_sa_edit #txt_judged_by_qc').prop('required',false);
			}
		});
	}
	...
	$('#tbl_special_acceptance tbody').on('click', 'tr .fa-edit', function(){
		...
		fn_validate_qc_supervisor();//modifynow
	...
	$('#btn_sa').click(function(){
		...
		re_initialize_select2_server_side('#modal_sa #txt_judged_by_qc','#modal_sa #frm_sa',[],"server_side_scripts/dropdown/common/dd_hris_above_ss_list.php");
		$('#modal_sa').data('id',0);
 ...
 */
	$('#btn_add_approver').click(function(e){ //feditcount
		...
		var html = '<tr>';
			...
			html += '<td style="width:15%;"><input class="form-control" type="number" min="1" step="1" name="row_coordinate[]" id="row_coordinate" value="48" style="width:100%" required></td>';
		...
		(row_count >5)? $(this).prop('disabled',true) : $(this).prop('disabled',false);
	});
	$('#edit_btn_add_approver').click(function(e){ //feditcount
		...
		var html = '<tr>';
			...
			html += '<td style="width:15%;"><input class="form-control" type="number" min="1" step="1" name="edit_row_coordinate[]" id="edit_row_coordinate" value="48" style="width:100%" required></td>';
			html += '<td style="width:15%;"><button type="button" class="btn btn-danger fa fa-remove" id="btn_remove"></button></td>';
			html += '</tr>';
	...
	$('#frm_sa').submit(function(e){
		...
		if(fn_validate_upload('judged_by_approver')){ /*NOTE : Approver is equal to 2*/
			 if(fn_validate_originator('txt_judged_by_qc')){ /*NOTE : MODIFY foriginator */ //amodifynow
	...
	$('#frm_sa_edit #replace_file').click(function(){
		$('#modal_sa_replace_file').modal('show');
	});
	$('#frm_sa_replace_file').submit(function(e){
		e.preventDefault();
		$('#tbl_approver_view tbody').empty();
		var form_data = new FormData(this);
			form_data.append("action","replace_sa_file");
			form_data.append("attachment_pkid",$('#modal_sa_edit').data('id'));
			form_data.append("username",username);
			fn_sa_replace_file(form_data);
	});
	$('#frm_sa_edit').submit(function(e){
		...
		if(fn_validate_edit('judged_by_approvers')){ //NOTE : Approver is only equal to 2
			if(fn_validate_originator_edit('txt_judged_by_qc')){ //amodify
 ...
 */	
	function default_approver_input(row_count){
		...
		var html = '<tr>';
			html += '<td style="width:3%;"><input class="form-control" type="text" value="'+add_row_count+'" name="order_id[]" id="order_id" readonly></td>';
			html += '<td style="width:45%;"><select class="form-control" data-placeholder="Select Approver Name" name = "pmi_approvers[]" id="pmi_approvers" value="ritamor" style="width: 100%" required></select></select></td>';
			html += '<td style="width:15%;"><input class="form-control" type="text" style="text-transform:uppercase" name="column_coordinate[]" id="column_coordinate" class="form-control" style="width:100%" required></td>';
			html += '<td style="width:15%;"><input class="form-control" type="number" min="1" step="1" name="row_coordinate[]" id="row_coordinate" value="48" style="width:100%" required></td>';
	...
	function append_approver_table_view(pmi_approvers,column,row,order_by){
			...
			var html = '<tr>';
				...
				html += '<td style="width:15%;"><input class="form-control" type="number" min="1" step="1" value = "'+row+'" name="edit_row_coordinate[]" id="edit_row_coordinate" value="48" style="width:100%" required></td>';
	...
	function fn_load_special_acceptance(pkid,mode,current_status){
		...
		call_ajax(data, handler_qfr, function(result){
			...
			$('#frm_sa_edit #container_uploaded_file').show();
			var status = result['status']; 
			if(status=="A"){
				$('#frm_sa_edit #badge_status').text('FOR QC CHECKING');
...
*/
			//modifynow
			assign_value_select2('#frm_sa_edit #txt_judged_by_qc',result['qc_approver_name']);
			re_initialize_select2_server_side('#modal_sa_edit #txt_judged_by_qc','#modal_sa_edit #frm_sa_edit',[],"server_side_scripts/dropdown/common/dd_hris_above_ss_list.php");
	...
	************************************** */
	...
	function fnMainValidateIsApprover(pkid,current_status){
		...
		$.ajax({
			...
			dataType: "json",
			success: function (result) {
				let approver_username = result['approver_username'];
				// let username = result['username'];
				if(current_status =='FOR APPROVAL'  && approver_username == username) {
					$('#frm_sa_edit  #btn_sa_main_approve').show();
					$('#frm_sa_edit  #btn_sa_main_disapprove').show();
				}else{
					$('#frm_sa_edit  #btn_sa_main_approve').hide('fast');
	...
	function fn_validate_upload(approver_id) {
		if($('#'+approver_id + ' option:selected').length > 2) {
				$('#modal_sa #container_upload_sa_message').html('Please select only 2 Checked / Approved by');
	...
	function fn_validate_edit(approver_id) {
		if($('#'+approver_id + ' option:selected').length > 2) {
			$('#modal_sa_edit #container_upload_sa_message').html('Please select only 2 Checked / Approved by');
	...
	function fn_validate_originator(judged_by_qc_id) {
		$judged_by_qc = $('#'+judged_by_qc_id + ' option:selected');
		if($judged_by_qc.length > 1 ) {
			$('#modal_sa #container_upload_sa_message').html('Please select only 1 qc supervisor.');
			$('#modal_sa #container_upload_sa_message').show();
			$('.btn').prop("disabled",false);
			return false;
		} else {
	...
	function fn_validate_originator_edit(judged_by_qc_id) {
		$judged_by_qc = $('#'+judged_by_qc_id + ' option:selected');
		if($judged_by_qc.length > 1 ) {
			$('#modal_sa_edit #container_upload_sa_message').html('Please select only 1 qc supervisor.');
			$('#modal_sa_edit #container_upload_sa_message').show();
			$('.btn').prop("disabled",false);
			return false;
		} else {
	...
	***************************************/
	...
	$('#frm_sa_edit_disposition #status').click(function (e) { 
		e.preventDefault();
		var status = $(this).val();
		fn_disposition_fields(status,'frm_sa_edit_disposition');
	});
	$('#frm_sa_edit_disposition #fa_paperclip').click(function (e) {
		var id = $('#frm_sa_edit_disposition #pkid').val();
		window.location.href = "reports/excel_qfr_sa_for_disposition_download.php?id="+id;
	});
	...
	$('#frm_sa_edit_disposition').submit(function (e) { 
		...
		call_ajax_attachment(serialized_data,handler_qfr,function(result){
			...
			dt_special_acceptance_disposition.draw();
			notif_success('Edit Successfully');
		});
