 ...
 * 
 * ! ****FUNCTION LIST****
 * function fn_get_machine_inspection_record
 * function fn_pp_oo_as_draw_row
 * function fn_pp_oo_return_visual_inspection_fields
 * function fn_pp_oo_advance_search
 * function fn_pp_return_machine_list
 * 
 * ! ****BUTTON LIST****
...
$(document).ready(function () {
	...
	$('#tbl_pre_production tbody').on('click', '.fa-edit', function() {
		...
		$('#frm_pp_update_inspection_result #measurescope_file').val('');
	...
	$('#tbl_pre_production tbody').on('click', '.fa-remove', function() {
		$('#modal_pp_delete').modal('show');
		var pkid = $(this).data('id');
		fn_get_machine_inspection_record('modal_pp_delete','form_pp_delete',pkid,'delete');
	});
	
	$('#form_pp_delete').submit(function (e) { 
		e.preventDefault();
		var serialized_data = $(this).serialize();
		fn_pp_delete_info(serialized_data);
	});
...
/* New - Start */
	...
	$('#btn_pp_new_inspection_main').click(function() {
		...
		re_initialize_select2_server_side('#'+mdl_id+' #cmb_checked_by','#'+mdl_id+' #'+frm_id,[],"server_side_scripts/dropdown/common/dd_rapid_hris_and_subcon_list.php");
		re_initialize_select2_server_side('#'+mdl_id+' #cmb_approved_by','#'+mdl_id+' #'+frm_id,[],"server_side_scripts/dropdown/common/dd_rapid_hris_and_subcon_list.php");
...
function fn_pp_delete_info(serialized_data){
	var data = {'action':'pp_delte_data'};
	call_ajax_serialize(data,serialized_data, handler_ipqc_pp, function(result){
		console.log(result);
		$('#modal_pp_delete').modal('hide');
		dt_ipqc_pp.draw();
