<?php
	...
	function load_special_acceptance(){ //NOTE : modify now
		require_once('../class/oop_tqts.php');
		$array_fields = array('*',"(SELECT `vw_role`.`role` FROM `vw_user_roles` `vw_role` WHERE `vw_role`.`user` = '".$_POST['username']."' AND `vw_role`.`module` = 'Special Acceptance' AND `vw_role`.`subsystem_code` = 'QFR' AND `vw_role`.`logdel` = '0') as role");
		...
		while($row = mysqli_fetch_array($result)){
			...
			$return['column_coordinate'] 	= $row['column_coordinate'];
			$return['row_coordinate'] 		= $row['row_coordinate'];
		...
		$joins 	   	= '';
		$sql_where 	= 'WHERE fkspecial_acceptance="'.$_POST['pkid'].'" AND logdel=0';
		$sql_order 	= '';
		$sql_limit 	= '';
		$result = TQTS::getInstance()->select_query($array_fields,$table,$joins,$sql_where,$sql_order,$sql_limit);
		$script= TQTS::getInstance()->select_query_script($array_fields,$table,$joins,$sql_where,$sql_order,$sql_limit);
		...
		if($return['ext'] == 'XLSX'){
			$return['attached_file'] 				= $return['file_path']['path'].'e_'.$_POST['pkid'].'.'.'xlsx';
			$return['attached_file_link'] 			= str_replace('../','',$return['attached_file']);
			
		}else{
			$return['attached_file'] 				= $return['file_path']['path'].'e_'.$_POST['pkid'].'.'.$return['ext'];
			$return['attached_file_link'] 			= str_replace('../','',$return['attached_file']);
		}
		...
		$joins 	   	= '';
		$sql_where 	= 'WHERE `fkid`="'.$_POST['pkid'].'" AND logdel="0"';
		$sql_order 	= '';
		$sql_limit 	= '';
		$result 	= TQTS::getInstance()->select_query($array_fields,$table,$joins,$sql_where,$sql_order,$sql_limit);
...
/**
 * !function to get the main approvers
 */
		$approver_username	= array();
		...
		while($row = mysqli_fetch_array($result)){
			$approver_username[]	= $row['approver_username'];
			$return['y_coordinate'][]  		= $row['y_coordinate'];
			$return['x_coordinate'][] 		= $row['x_coordinate'];
			$return['order_id'][] 		= $row['order_id'];
		}
		$implode_approver_username 	= implode(',',$approver_username);
		$array_pmi_approver = explode(",",$implode_approver_username);
		foreach($array_pmi_approver as $key => $value){
			$array_data_app 				= array();
			$array_data_app['id'] 			= $value;
			$array_data_app['text'] 		= get_emp_name_by_username_systemone($value);
		...
		$return_qc_approver = load_qc_approver($pkid);
		foreach ($return_qc_approver as $key => $value) {
			$array_data_app 				= array();
			$array_data_app['id'] 			= $value;
			$array_data_app['text'] 		= get_emp_name_by_username_systemone($value);
			$return['qc_approver_name'][]	= $array_data_app;
		}
		//modifynow
		echo json_encode($return);
 ...
 */
	...
	function save_special_acceptance(){ //xmodifynow
		require_once('../class/oop_tqts.php');

		$return 		= $_POST;
		$order_id = implode(',',$return["order_id"]); 
		$column = implode(',',$return["column_coordinate"]); 
		$str_column  = strtoupper($column);
		$row = implode(',',$return["row_coordinate"]); 
		$qc_approvers = isset($return['judged_by_qc'])?$return['judged_by_qc']:array('');
		$qc_approvers = implode(',',$qc_approvers);
		$pmi_approvers = implode(',',$return["pmi_approvers"]); 
		$approvers = implode(',',$return["judged_by_approver"]);
		...
		if($return['ext'] == "XLSX"){
			move_uploaded_file($return['temp_name'],$directory_path['path'].'e_'.$pkid.'.xlsx');
		}else{
			move_uploaded_file($return['temp_name'],$directory_path['path'].'e_'.$pkid.'.'.$return['ext']);
		}

		/** when you save special acceptance, the default status is zero(0) - FOR CHECKING */
		/** but if the qc approvers is exist, the status will change into A - FOR QC CHECKING*/
		save_qc_approvers($username,$pkid,$qc_approvers,$date_time_today);
...
*/		
		send_email_for_checking($return['pkid']); //bamail

		$approvers 		 = explode(',',$pmi_approvers);
		$column = explode(',',$str_column); 
		$row = explode(',',$row); 
		$order_id = explode(',',$order_id); 

		for($i=0;$i<count($approvers);$i++) {
			$approver_username 	= $approvers[$i];
			$return_column = $column[$i];
			$return_row = $row[$i];
			$return_order_id = $order_id[$i];
			/* Save 4 main approvers */
			$table			= 'tbl_qfr_sa_approvers_main';
			$array_fields 	= array('created_by','fkid','order_id', 'approver_username','status','y_coordinate','x_coordinate','created_at', 'updated_at', 'username');
				$array_values 	= array($username, $pkid,$return_order_id, $approver_username, '0',$return_column,$return_row,$date_time_today,$date_time_today, $username);
				$insert_query	= TQTS::getInstance()->insert_query($table,$array_fields,$array_values);
		}
		/* SAVE KO DITO YUNG EMPNAME GAMIT YUNG FUNCTION SA COMMON FUNCTION - return_esign_by_username */
		/* attachment */
		$table			= 'tbl_qfr_special_acceptance_attachment';
		$array_fields 	= array(
									'date_time_created','created_by','fkspecial_acceptance',
									'file_name','fkfile_path','reason_delete',
									'lastupdate','username'
								);
		$array_values	= array(
									date('Y-m-d H:i:s'),$return['username'],$pkid,
									$return['filename'],4,'',
									date('Y-m-d H:i:s'),$return['username']
								);
	...
	function edit_special_acceptance(){ //NOTE : feditcount
		require_once('../class/oop_tqts.php');
		$date_time_today = date('Y-m-d H:i:s');
		$return 		= $_POST;
		$return['error']= array();	
		$pkid			= $return['pkid'];
		$str_column = implode(',',$return["edit_column_coordinate"]); 
		$column_coordinate  = strtoupper($str_column);
		$row_coordinate = implode(',',$return["edit_row_coordinate"]); 
		
		$qc_approvers = isset($return['judged_by_qc'])?$return['judged_by_qc']:array('');
		$qc_approvers = implode(',',$qc_approvers);
		$has_record = load_is_qc_approver($pkid);
		$new_status = ($has_record == 1)?'A':'0'; /**check the if the qc approvers is exist*/
		//Pmi Approvers
		$pmi_approvers = implode(',',$return["edit_main_approvers"]); 
		$approvers = implode(',',$return["judged_by_approvers"]); 
		$order_id = implode(',',$return["edit_order_id"]); 
		$username = $return ['username'];
		...
		$array_fields 	= array(
								...
								'drawing_number','other_details','judged_by_qc'
								,'judged_by_approver','pmi_approvers','column_coordinate',
								'row_coordinate','lastupdate','status'
								);
		...
		$array_values	= array(
								...
								$return['drawing_number'],$return['other_details'],$qc_approvers,$approvers,$pmi_approvers,
								$column_coordinate,$row_coordinate,date('Y-m-d H:i:s'),$new_status
								);
		$where 			= "WHERE pkid = '$pkid'";
		
		$result 			= TQTS::getInstance()->update_query_detailed($table,$array_fields,$array_values,$where);
		/** 
		 Update QC Approver,Checked by(Manager or Senior SV), PMI Approvers(Section Head)
		*if qc_approver exist/defined this will allowed to edit the qc approvers, if not: the qc cannot edit the qc approvers.
		* in short, the current login is ENGG, not QC
		*/ //bamail
		send_email_for_checking($pkid);

		// QC approver
		$return_qc_approver = load_qc_approver($pkid);
		if($return_qc_approver['qc_approver_name'] != "NONE"){
			edit_qc_approvers($pkid,$qc_approvers,$username,$date_time_today);
		}
		edit_approvers($pkid,$approvers,$_POST['username'],$date_time_today);
		edit_pmi_approvers($pkid,$pmi_approvers,$_POST['username'],$column_coordinate,$row_coordinate,$date_time_today,$order_id);
		echo json_encode($result); //hmodifynow
	}
	function edit_approvers($pkid,$approvers,$username,$date_time_today){
		$table			= 'tbl_qfr_sa_approvers';
	...
	function edit_pmi_approvers($pkid,$pmi_approvers,$username,$column_coordinate,$row_coordinate,$date_time_today,$order_id){
		$table			= 'tbl_qfr_sa_approvers_main';
		$array_fields 	= array('updated_at', 'username', 'logdel');
		$array_values 	= array($date_time_today, $username, 1);
		$sql_where		= 'WHERE fkid='.$pkid;
		// $sql_where		= "WHERE `fkid`='".$pkid."' AND `status` = 0"; /* additional: if updating the approvers, the approved wil not edited | solution: if the approvers is existed and status =1 dont save */
		$update_query	= TQTS::getInstance()->update_query_detailed($table,$array_fields,$array_values,$sql_where);
		$array_fields 	= array('created_at', 'created_by', 'fkid', 'approver_username', 'status','y_coordinate','x_coordinate', 'updated_at', 'username');
		
		$approvers 		 = explode(',',$pmi_approvers);
		$column = explode(',',$column_coordinate); 
		$row = explode(',',$row_coordinate); 
		$order_id = explode(',',$order_id); 
		for($i=0;$i<count($approvers);$i++) {
			$approver_username 	= $approvers[$i];
			$return_column = $column[$i];
	...
	// function validate_main_is_approver(){
	// 	require_once('../class/oop_tqts.php');
